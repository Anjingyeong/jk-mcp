...
export async function codeSearch(
  ...
  const excluded = normalizeExcludedRoots(realRoot, excludeRoots);
  const rgResult = await tryRipgrep(realRoot, query, cap, excluded);
  if (rgResult) return rgResult;

  const matches = await jsFallbackSearch(realRoot, query, cap, excluded);
...
async function tryRipgrep(
  ...
  query: string,
  cap: number,
  excluded: Array<{ abs: string; rel: string }>,
...
function normalizeExcludedRoots(root: string, excludeRoots: string[]): Array<{ abs: string; rel: string }> {
  const out: Array<{ abs: string; rel: string }> = [];
  for (const candidate of excludeRoots) {
    const abs = path.resolve(candidate);
    const relNative = path.relative(root, abs);
    if (!relNative || relNative === ".." || relNative.startsWith(`..${path.sep}`) || path.isAbsolute(relNative)) continue;
    const rel = relNative.split(path.sep).join("/");
    if (out.some((item) => abs === item.abs || abs.startsWith(item.abs + path.sep))) continue;
    out.push({ abs, rel });
  }
  return out;
}
...
async function walk(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (excluded.some((item) => next === item.abs)) continue;
      await walk(root, onFile, next, excluded);
