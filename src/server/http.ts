...
import { registerRoleManagementRoutes } from "../roles/http.js";
import { registerControlCenterRoutes } from "../control-center/http.js";
import { registerExecutorRoutes } from "../executors/http.js";
...
export interface HttpServerConfig {
  ...
  onIdleTimeout?: () => void;
  /** Register the optional human management UI/API surface. Core MCP/actions do not depend on it. */
...
export function createHttpServer(ctx: ToolContext, config: HttpServerConfig): RunningHttpServer {
  ...
  const remoteManagementHost = (process.env.JK_REMOTE_MANAGEMENT_HOST ?? "").trim().toLowerCase().split(":", 1)[0] ?? "";
  ...
  const allowedHostnames = Array.from(
    new Set([
      publicUrl.hostname,
      ...loopbackHosts,
      ...(remoteManagementHost ? [remoteManagementHost] : []),
  ...
  // Host-local reverse proxies may terminate on loopback and forward
  // X-Forwarded-For; trust only loopback proxies so rate-limit keys clients.
  ...
  const allowedOrigins = new Set<string>([
    ...
    ...(remoteManagementHost ? [`https://${remoteManagementHost}`] : []),
  ...
  if (config.managementRoutesEnabled) {
    registerControlCenterRoutes(app, ctx);
    registerRoleManagementRoutes(app, ctx);
  }
  registerExecutorRoutes(app, ctx);
  registerActionRoutes(app, ctx, publicUrl, oauthProvider, config.oauth.scopes[0] ?? "chatgpt2codex");
