...
function gitRequired(root,args){ const r=cp.spawnSync("git",["-C",root,...args],{encoding:"utf8",stdio:["ignore","pipe","pipe"],windowsHide:true,maxBuffer:4*1024*1024}); if(r.status!==0) throw new Error("git "+args.join(" ")+" failed: "+String(r.stderr||r.stdout||"").trim()); return String(r.stdout||"").trim(); }
...
function gitSyncStart(root){ const before=repoStatus(root); if(!before.branch) throw new Error("Remote-work sync requires a git branch"); if(!before.upstream) throw new Error("Remote-work sync requires a configured upstream branch"); if(before.dirtyFiles.length||before.staged.length) throw new Error("Refusing remote-work sync because the checkout is not clean"); const remote=upstreamRemote(before.upstream); gitRequired(root,["fetch","--prune",remote]); const fetched=repoStatus(root); if(fetched.ahead>0&&fetched.behind>0) throw new Error("Refusing remote-work sync because local and upstream histories diverged"); if(fetched.ahead>0) throw new Error("Refusing remote-work sync because the remote worker has commits that are not on upstream"); let fastForwarded=false; if(fetched.behind>0){ gitRequired(root,["merge","--ff-only",fetched.upstream||before.upstream]); fastForwarded=true; } const after=repoStatus(root); if(after.ahead!==0||after.behind!==0||after.dirtyFiles.length||after.staged.length) throw new Error("Remote-work sync did not finish in a clean upstream-aligned state"); return {branch:after.branch,upstream:after.upstream||before.upstream,remote,before,after,fastForwarded}; }
