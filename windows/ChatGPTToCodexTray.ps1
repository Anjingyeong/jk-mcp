...
$RepoUrlDefault = if ($env:CHATGPT2CODEX_UPDATE_REPO_URL) { $env:CHATGPT2CODEX_UPDATE_REPO_URL } else { "https://github.com/Anjingyeong/jk-mcp" }
$StartupShortcutPath = Join-Path ([Environment]::GetFolderPath("Startup")) "JK.lnk"
...
$Localization = @{
    ...
    settingsTitle = @("JK Settings", "JK 설정", "JK 設定", "JK 设置", "JK 設定", "Ajustes de JK", "Réglages de JK", "JK Einstellungen", "Configurações do JK", "Impostazioni JK", "JK instellingen", "Ustawienia JK", "Настройки JK", "JK ayarları", "Cài đặt JK", "Pengaturan JK", "การตั้งค่า JK", "إعدادات JK", "JK सेटिंग्स", "Налаштування JK")
    ...
    upToDate = @("JK is up to date ({0}).", "JK가 최신입니다 ({0}).", "JK は最新です ({0})。", "JK 已是最新版本（{0}）。", "JK 已是最新版本（{0}）。", "JK está actualizado ({0}).", "JK est à jour ({0}).", "JK ist aktuell ({0}).", "JK está atualizado ({0}).", "JK è aggiornato ({0}).", "JK is up-to-date ({0}).", "JK jest aktualny ({0}).", "JK обновлен ({0}).", "JK güncel ({0}).", "JK đã mới nhất ({0}).", "JK sudah terbaru ({0}).", "JK เป็นเวอร์ชันล่าสุด ({0})", "JK محدث ({0}).", "JK अप टू डेट है ({0})।", "JK оновлено ({0}).")
    ...
    aboutInfo = @("JK`nLocal coding bridge for ChatGPT`n`nBased on ChatGPT To Codex by ezBuilder.`nOriginal work © 2026 ezBuilder. All rights reserved.", "JK`nChatGPT용 로컬 코딩 브리지`n`nChatGPT To Codex by ezBuilder를 기반으로 합니다.`n원본 저작물 © 2026 ezBuilder. All rights reserved.")
}
...
function Get-LauncherPath {
    $exe = Join-Path $RuntimeRoot "JK.exe"
...
function Start-Service {
    ...
    Append-Log "`n[$(Get-Date -Format s)] starting JK..."
...
function Show-Settings {
    ...
    $copyright.Text = "Original work © 2026 ezBuilder. All rights reserved."
    $copyright.SetBounds(148, 437, 340, 22)
...
$notify.Text = $AppName
$iconIco = Join-Path $RuntimeRoot "JK.ico"
$iconPath = Join-Path $RuntimeRoot "assets\jk-icon.png"
...
$statusItem = [System.Windows.Forms.ToolStripMenuItem]::new("JK: $(L "statusChecking")")
