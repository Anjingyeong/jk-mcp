...
- shell을 실행하지 않고 호환되는 Codex cache 및 전역 OMO Native 설치 탐색
- legacy `omo run --help` 계약과 OMO Native print 계약을 모두 검사
- 가장 최신의 호환 legacy 버전 또는 shell-free 전역 native Node entry 선택
- 최신 버전이 깨졌다면 설치된 이전 호환 버전으로 자동 fallback
- 프롬프트를 shell 문자열이 아닌 argv로 전달
- legacy 실행에서 agent를 지정하지 않으면 `general`을 기본값으로 사용
- OMO session ID를 회수해 후속 실행에 재사용 가능

JK는 다음 두 CLI 계약을 지원합니다.
...
`omo_run` 도구에서 선택적 `ultrawork: true`를 지정할 수 있습니다. JK는 native `ulw` trigger만 추가하고 숨겨진 directive 주입은 OMO에 맡기며, directive 본문을 복사하지 않습니다. ultrawork를 생략하거나 false로 지정하면 원본 메시지 바이트를 보존합니다. OMO Native에서는 `--omo-senpi-ultrawork-disabled`도 전달하므로 `mass-ulw` 같은 부수 텍스트가 hook을 활성화하지 않습니다. OMO Native print mode에는 agent 선택 기능이 없으므로, JK는 명시적 `agent`를 조용히 무시하지 않고 오류로 거부합니다.
